...
1. **Given** I have added 3 tasks, **When** I select the "View Tasks" option, **Then** I see all 3 tasks listed with their IDs, titles, and status (complete/incomplete)
...
1. **Given** I have a task with ID 3, **When** I select "Update Task" and provide ID 3 with new description "Updated details here", **Then** the task's description is updated and confirmation is shown
2. **Given** I update a task's description, **When** I view the task, **Then** I see the new description while the title and ID remain unchanged
3. **Given** I attempt to update a non-existent task ID, **When** I submit the ID and description, **Then** I see an error message indicating the task was not found
4. **Given** I provide an empty description, **When** I update the task, **Then** the description is cleared (set to empty)
...
- **Completing already completed task**: Should either be idempotent (no error, already complete) or show informational message
- **Menu selection case sensitivity**: Menu options should accept input case-insensitively (e.g., "quit", "QUIT", "Quit" all work)

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST display an interactive menu with options: Add Task, View Tasks, Update Task, Delete Tasks, Mark Complete, Quit
- **FR-002**: System MUST return to the main menu after completing any operation (add, view, update, delete, mark complete)
- **FR-003**: System MUST allow users to view all tasks showing ID, title, and status (complete/incomplete) for each task
...
- **FR-010**: System MUST allow users to update a task's description by providing the task ID and new description text
- **FR-011**: System MUST preserve the task title and ID when updating description
...
- **FR-016**: System MUST handle invalid menu selections and re-display the menu with an error message
- **FR-017**: System MUST accept menu options in a case-insensitive manner
...
- **Menu**: Represents the interactive interface with options:
  ...
  - View Tasks (displays all tasks with ID and status)
...
- Marking an already-completed task as complete is idempotent (no error, informational message optional)
- Menu prompts use numbered options (1. Add Task, 2. View Tasks, etc.) or text commands (add, view, etc.) - either is acceptable
