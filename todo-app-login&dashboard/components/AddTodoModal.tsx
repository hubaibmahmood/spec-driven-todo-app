...
const AddTodoModal: React.FC<AddTodoModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  const handleAiEnhance = async () => {
    ...
    setIsEnhancing(true);
    try {
      const enhanced = await enhanceTaskContent(title);
      setTitle(enhanced.title);
      setDescription(enhanced.description);
      setPriority(enhanced.priority);
      setTags(enhanced.tags.join(', '));
    } catch (error) {
      console.error("Enhancement failed", error);
      alert("Failed to enhance task. Ensure API Key is set.");
    } finally {
